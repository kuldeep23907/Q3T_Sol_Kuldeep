...
describe('agent-minter', () => {
  ...
  const program = anchor.workspace
    .AgentMinter as Program<AgentMinter>;
  ...
  it('InitializeContext!', async () => {
    ...
    const counter = await program_llm.account.counter.fetch(
      counterAddress
    );
    ...
    console.log('llmContext', contextAddress);
  ...
  it('InteractAgent!', async () => {
    const { agent, interactionAddress } =
      await GetAgentAndInteraction(program, provider, program_llm);

    const tx = await program.methods
      .interactAgent(
        'Diwali NFT',
        'DIYA',
        'new,home,diwali,lights,celebration'
      )
      .accounts({
        payer: provider.wallet.publicKey,
